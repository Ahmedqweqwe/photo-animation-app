...
# أضفنا مفتاح ثابت (key="image_uploader") لمنع أخطاء الواجهة أثناء إعادة التحميل
uploaded_file = st.file_uploader("اختر صورة...", type=["jpg", "jpeg", "png", "webp"], key="image_uploader")
...
if uploaded_file is not None:
    ...
    # تصغير حجم الصورة تلقائياً لحماية رامات الاستضافة
    max_size = 600
    ...
    # استخدام st.empty() لإنشاء حاوية ديناميكية آمنة للمتصفح
    status_container = st.empty()
    video_container = st.empty()
    
    # أضفنا key="submit_btn" للزر لتفادي مشكلة removeChild
    if st.button("تطبيق المؤثرات وتوليد الفيديو ✨", key="submit_btn"):
        
        # وضع الـ spinner داخل الحاوية الآمنة
        with status_container.container():
            with st.spinner("جاري معالجة الصورة وإضافة التوهج النبضي... انتظر قليلاً"):
                ...
                for frame_num in range(total_frames):
                    ...
                    glow_layer = cv2.multiply(glow_layer, np.array([pulse, pulse * 0.9, pulse * 1.1]))
        ...
        # تنظيف حاوية الحالة السابقة لتهيئة المتصفح
        status_container.empty()
        
        # عرض النجاح والفيديو داخل الحاوية المخصصة بشكل مستقر
